...
UI_TAB_TITLE = "Виртуальный ассесор"
        ...
        def yield_deltas(completion_gen: CompletionGen) -> Iterable[str]:
            ...
            if completion_gen.sources:
                ...
                for index, source in enumerate(cur_sources, start=1):
                    file_name = string_replacing(source.file)
                    if f"{file_name}" not in used_files:
                        sources_text = (
                            sources_text
                            + f"{index}. {file_name} \n\n"
                        )
                        used_files.add(f"{file_name}")
                full_response += sources_text
            yield full_response

        def string_replacing(file_name: str):
            file_name = file_name.replace(".docx", "")
            file_name = file_name.replace(".csv", "")
            if "lesson" in file_name:
                file_name = file_name.replace("lesson_", "Урок ")
            if "lection" in file_name:
                file_name = file_name.replace("lection_", "Лекция ")
            if "train" in file_name:
                file_name = file_name.replace("train_data.csv", "")
            return file_name
